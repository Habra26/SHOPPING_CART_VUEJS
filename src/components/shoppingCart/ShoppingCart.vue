    ...
    <div class="bg-gray-50">
      <div class="p-6">
        ...
        <div class="flex justify-between mb-1">
          ...
          <span>€{{ shoppingCartStore.tva.toFixed(2) }}</span>
        </div>
        <div class="flex justify-between items-center my-8">
          <span><strong>Livraison:</strong></span>
          <select
            class="form-select py-1 px-2 block w-full rounded border border-gray-300 ml-2"
            @change="shoppingCartStore.shipping = Number($event.target.value)"
          >
            <option value="0">-- Choisissez un mode de livraison --</option>
        ...
        <div class="flex justify-between font-bold mb-1 text-xl">
          ...
          <span>€{{ shoppingCartStore.totalGeneral.toFixed(2) }}</span>
        </div>
        <button
